...
async function main() {
    try {
        ...
        const currentUsage = await (0, aiCostTracker_1.getCurrentMonthUsage)();
        ...
        if (apiKey && apiKey.length > 0) {
            ...
            try {
                const testReply = await (0, ai_1.aiReply)({
                    role: 'cliente',
        ...
        const usageAfter = await (0, aiCostTracker_1.getCurrentMonthUsage)();
        ...
        const exampleCost = (0, aiCostTracker_1.calculateCost)('gpt-4o-mini', 100, 50);
        console.log(`   💰 100 tokens input + 50 tokens output = $${exampleCost.toFixed(6)} USD\n`);
        console.log('✅ Prueba completada exitosamente!\n');
    }
    catch (error) {
        const msg = error?.message ?? String(error);
        logger_1.default.error('Error en prueba de costos de IA:', msg);
